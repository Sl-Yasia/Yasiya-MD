...
async function startMiku() {
    ...
    console.log(color(figlet.textSync('JesiQueen Bot MD', {
        font: 'Pagga',
    ...
    console.log(color('\n🙇 Thanks for using: JesiQueen MD Made by Yasiya.'))
    ...
    Miku.ev.on('connection.update', async (update) => {
        ...
        status = connection;
        if (connection) {
            await console.info(`JesiQueen MD Server Status => ${connection}`);
          }
